...
def bndry():
    ...
    u[0, 0, 0] = 2*u[1, 0, 0] - u[2, 0, 0] # lower left corner point pressure
    u[imax - 1, 0, 0] = 2*u[imax - 2, 0, 0] - u[imax - 3, 0, 0] # lower right corner point pressure
...
def compute_time_step(dtmin):
    ...
    lambda_x = 0.5*(np.abs(u[1:imax - 1, 1:jmax - 1, 1]) + np.sqrt(uvel2 + 4*beta2))
